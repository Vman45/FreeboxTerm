 ...
 */
export const authorize = (uid, dl) => {
  const app_token = store.getState().config.getIn(['tokens', uid])
  if (!app_token) { return register(uid, dl) }
  return login(app_token, uid, dl)
}
 ...
 */
const login = (app_token, uid, dl) => {

  if (!dl) { start('Login in') }
 ...
 */
const register = (uid, dl) => {

  const { app_id, app_name, app_version } = config

  if (!dl) { start('Waiting for authorization') }
